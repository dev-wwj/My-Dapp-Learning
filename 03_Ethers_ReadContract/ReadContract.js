...
const addressDAI = '0x00ab7822383F23B5801D427f4e13c667c0a68773'
...
const main = async() => {
    ...
    console.log(`Contract Name:    ${nameWETH}`)
    console.log(`Contract Symbol:  ${symbolWETH}`)
    console.log(`Contract Total:   ${ethers.formatEther(totalSupplyWETH)}`)

    const balanceWETH = await contractWETH.balanceOf('vitalik.eth')
    console.log(`Vitalik's Balance ${ethers.formatEther(balanceWETH)}\n`)

    // 2. 读取DAI合约的链上信息（IERC20接口合约）
    const nameDAI = await contractDAI.getCount()

    console.log("\n2. read Contract DAI Infomation")
    console.log(`Contract Address: ${addressDAI}`)
    console.log(`Contract Nmae: ${nameDAI}`)
